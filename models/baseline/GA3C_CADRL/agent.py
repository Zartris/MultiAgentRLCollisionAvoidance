...
import models.baseline.GA3C_CADRL.util as util
import operator
...
class Agent():
    def __init__(self, start_x, start_y, goal_x, goal_y, radius=0.5, pref_speed=1.0, initial_heading=0.0, id=0):
        ...
        if Config.ROBOT_MODE:
            ...
            self.time_remaining_to_reach_goal = 4 * np.linalg.norm(
                self.pos_global_frame - self.goal_global_frame) / self.pref_speed
        else:
            self.time_remaining_to_reach_goal = 2 * np.linalg.norm(
                self.pos_global_frame - self.goal_global_frame) / self.pref_speed
        self.t = 0.0
    ...
    def update_state(self, action, dt):
        ...
        if self.action_time_lag > 0:
            ...
            if timestamp_of_action_to_execute < 0:
                ...
                nearest_timestamp, _ = util.find_nearest(np.array(self.chosen_action_dict.keys()),
    ...
    def observe(self, agents: List['Agent']):
        ...
        obs[
        Config.AGENT_ID_LENGTH + Config.FIRST_STATE_INDEX:Config.AGENT_ID_LENGTH + Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE] = \
        ...
        for other_agent in clipped_sorted_agents:
            ...
            end_index = Config.AGENT_ID_LENGTH + Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * (
                    i + 1)

            other_obs = np.array(
                [p_parallel_ego_frame, p_orthog_ego_frame, v_parallel_ego_frame, v_orthog_ego_frame, other_agent.radius, \
