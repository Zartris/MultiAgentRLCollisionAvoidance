...
from models.baseline.GA3C_CADRL.config import Config
...
def convert_cadrl_state_to_state(cadrl_state):
    ...
    if Config.MAX_NUM_OTHER_AGENTS_OBSERVED in [3, 4]:
        ...
        for i in range(Config.MAX_NUM_OTHER_AGENTS_OBSERVED):
            ...
            agent_state[
                is_on_inds, Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 0 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i] = \
                cadrl_state[is_on_inds, 9 + 8 * i]  # other agent px
            agent_state[
                is_on_inds, Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 1 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i] = \
                cadrl_state[is_on_inds, 10 + 8 * i]  # other agent py
            agent_state[
                is_on_inds, Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 2 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i] = \
                cadrl_state[is_on_inds, 7 + 8 * i]  # other agent vx
            agent_state[
                is_on_inds, Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 3 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i] = \
                cadrl_state[is_on_inds, 8 + 8 * i]  # other agent vy
            agent_state[
                is_on_inds, Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 4 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i] = \
                cadrl_state[is_on_inds, 11 + 8 * i]  # other agent radius
            agent_state[
                is_on_inds, Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 5 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i] = \
                cadrl_state[is_on_inds, 12 + 8 * i]  # combined radius
            agent_state[
                is_on_inds, Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 6 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i] = \
                cadrl_state[is_on_inds, 13 + 8 * i]  # dist_2_other
            if Config.MULTI_AGENT_ARCH in ['WEIGHT_SHARING', 'VANILLA']:
                agent_state[
                    is_on_inds, Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 7 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i] = \
                    cadrl_state[is_on_inds, 14 + 8 * i]  # is_on
...
def plot_current_state_ego_frame(state, figure_name=None, axes=None):
    ...
    try:
        ...
        if Config.MULTI_AGENT_ARCH == 'RNN':
            ...
            num_others = int(sum([state[
                                      Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * (
                                              i + 1) - 1] for i in range(Config.MAX_NUM_OTHER_AGENTS_OBSERVED)]))

        for i in range(num_others):
            other_pxs.append(state[
                                 Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 0 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i])
            other_pys.append(state[
                                 Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 1 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i])
            other_vxs.append(state[
                                 Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 2 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i])
            other_vys.append(state[
                                 Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 3 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i])
            other_radii.append(state[
                                   Config.FIRST_STATE_INDEX + Config.HOST_AGENT_STATE_SIZE + 4 + Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH * i])
    ...
    wedge = ptch.Wedge([-host_dist_to_goal, 0.0], 1.0, rad2deg(host_heading - np.pi / 3),
                       rad2deg(host_heading + np.pi / 3), alpha=0.1)
    ax.add_patch(wedge)
    heading = ax.plot([-host_dist_to_goal, -host_dist_to_goal + np.cos(host_heading)], [0.0, np.sin(host_heading)],
                      'k--')
    ...
    for i in range(len(other_pxs)):  # plot all agents that are "ON"
        ...
        circ = plt.Circle((-host_dist_to_goal + other_pxs[i], other_pys[i]), radius=other_radii[i], fc='w',
                          ec=plt_colors_local[i + 1])
        ax.add_patch(circ)
        # other agent's speed
        ax.arrow(-host_dist_to_goal + other_pxs[i], other_pys[i], other_vxs[i], other_vys[i],
...
def plot_snapshot(state, real_action_one_hot, real_value, possible_actions, probs, values, figure_name=None):
    ...
    wedge = ptch.Wedge([-host_dist_to_goal, 0.0], 1.0, rad2deg(host_heading - np.pi / 3),
                       rad2deg(host_heading + np.pi / 3), alpha=0.1)
    ax.add_patch(wedge)
    heading = plt.plot([-host_dist_to_goal, -host_dist_to_goal + np.cos(host_heading)], [0.0, np.sin(host_heading)],
                       'k--')
    ...
    for i in range(len(other_pxs)):  # plot all agents that are "ON"
        ...
        ax.add_patch(circ)
        # other agent's speed
        plt.arrow(-host_dist_to_goal + other_pxs[i], other_pys[i], other_vxs[i], other_vys[i],
                  fc=plt_colors_local[i + 1], \
                  ec=plt_colors_local[i + 1], head_width=0.05, head_length=0.1)
    ...
    plt.scatter(plot_x, plot_y, marker='+', s=1000, linewidths=4, c=plot_z, cmap=plt.cm.rainbow, vmin=color_min,
                vmax=color_max)
