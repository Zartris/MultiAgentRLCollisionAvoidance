...
class PPOLoss(LossModule):
    ...
    # Declared so torchrl 0.11's `convert_to_functional` finds them in
    # `self.__class__.__annotations__` and doesn't emit a UserWarning each time.
    # Mirrors the annotations on upstream torchrl.objectives.PPOLoss.
    ...
    def loss_critic(self, tensordict: TensorDictBase) -> torch.Tensor:
        ...
        if self.clip_value:
            try:
                old_state_value: torch.Tensor = tensordict.get(self.tensor_keys.value)
            except KeyError:
                raise KeyError(
        ...
        # Require the padding mask explicitly. The previous fallback (scalar 0.0) hid
        # bugs where a scenario stopped emitting is_padding and the loss silently
        # stopped masking — or worse, a scenario emitted ones and the multiply zeroed
        # every entry, killing the gradient signal.
        padding_key = ("next", "agents", "info", "is_padding")
        if padding_key not in tensordict.keys(include_nested=True):
            raise KeyError(
                f"Expected {padding_key} in tensordict for critic loss padding mask. "
                "Scenarios must emit is_padding in their info dict."
            )
        is_padding = tensordict.get(padding_key)
        target_return = target_return * (1 - is_padding)
        state_value = state_value * (1 - is_padding)
...
class ClipPPOLoss(PPOLoss):
    ...
    # Python's class __annotations__ don't inherit, so we restate them here
    # even though PPOLoss already declares the same six names. Without this,
    # torchrl 0.11's convert_to_functional sees an empty __annotations__ on
    # ClipPPOLoss and warns six times on every construction.
    actor_network: TensorDictModule
    critic_network: TensorDictModule
    actor_network_params: TensorDictParams
    critic_network_params: TensorDictParams
    ...
    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        # Same hard-require as the critic path above — no silent fallback.
        padding_key = ("next", "agents", "info", "is_padding")
        if padding_key not in tensordict.keys(include_nested=True):
            raise KeyError(
                f"Expected {padding_key} in tensordict for advantage padding mask. "
                "Scenarios must emit is_padding in their info dict."
            )
        is_padding = tensordict.get(padding_key)
        advantage = advantage * (1 - is_padding)
